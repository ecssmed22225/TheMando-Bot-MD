import fetch from 'node-fetch';
import cheerio from 'cheerio';

let handler = async (m, { conn, command, text, usedPrefix }) => {
  if (!text) throw `*╮┄╌〔 ≪ سونيك البوت ≫ 〕╌╌•*\n*┆❌ بتستخدم الامر غلط*\n*┆✔️ الاستخدام الصح*\n*┆❕ مثال: ${usedPrefix + command} هيسوكا*\n*╯────ׂ─ׂ─ׂ─ׂ─────╌─╌─╌*`;
  m.react('⌛');

  try {
    const hasil = await pinterest(text);
    let gambarUrls = hasil.slice(0, 20); // اختيار 20 صورة أولية

    // خلط الصور عشوائياً
    for (let i = gambarUrls.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [gambarUrls[i], gambarUrls[j]] = [gambarUrls[j], gambarUrls[i]];
    }

    // إعداد قائمة الرسائل
    const messages = [];
    for (let i = 0; i < 10; i++) {
      let imageUrl = gambarUrls[i];
      let imageRes = await fetch(imageUrl);
      let imageBuffer = await imageRes.buffer();

      messages.push([
        `صورة ${i + 1}`,
        '𝑇𝐻𝐸 𝑺𝑶𝑵𝑬𝑲 𝑬𝑹𝑬𝑲',
        imageBuffer,
[['','']],[],[['لـيـنـك شـاتـي​​​​​​​🐦','https://wa.me/201503742544']] 

      ]);

      // إضافة تأخير لمنع إرسال الصور بسرعة كبيرة
      await new Promise(resolve => setTimeout(resolve, 500));
    }

    // إرسال الصور كـ Carousel
    await conn.sendCarousel(m.chat, '*╮┄╌〔 ≪ سسؤنيك آلبؤت ≫ 〕╌╌•*\n*┆✔️ تـم الـعـثـور عـلـي نـتـائـج*\n*╯────ׂ─ׂ─ׂ─ׂ─────╌─╌─╌*', '𝑇𝐻𝐸 𝐸𝐿-𝐷𝐸𝑺𝑂𝐾𝐸𝐼𝑌 𝐵𝑂𝑇', 'IMAGES', messages, m);
    m.react('✅') 
  } catch (e) {
    console.log(e);
    conn.reply(m.chat, '*╮┄╌〔 ≪ سسؤنيك آلبؤت ≫ 〕╌╌•*\n*┆❌ حدث خطأ غير متوقع*\n*╯────ׂ─ׂ─ׂ─ׂ─────╌─╌─╌*', m);
    m.react('❌')
  }
};

handler.help = ['pinterest3'];
handler.tags = ['downloader'];
handler.command = /^صور$/i;
handler.diamond = true;
handler.limit = 3;

export default handler;

async function pinterest(text) {
  try {
    const response = await fetch(`https://id.pinterest.com/search/pins/?autologin=true&q=${encodeURIComponent(text)}`, {
      headers: {
        "cookie": "_auth=1; _b=\"AXOtdcLOEbxD+qMFO7SaKFUCRcmtAznLCZY9V3z9tcTqWH7bPo637K4f9xlJCfn3rl4=\"; _pinterest_sess=; _ir=0"
      }
    });
    const data = await response.text();
    const $ = cheerio.load(data);
    const result = [];
    const hasil = [];
    $('div > a').get().map(b => {
      const link = $(b).find('img').attr('src');
      result.push(link);
    });
    result.forEach(v => {
      if (v && v.includes('236')) {
        hasil.push(v.replace(/236/g, '736'));
      }
    });
    hasil.shift();
    return hasil;
  } catch (error) {
    throw error;
  }
}
